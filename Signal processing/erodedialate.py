...
title_trackbar_gaussian = 'Blur size:\n 2n +1'
...
def main(image):
    ...
    src = gammaCorrection(src, 1.5)

    cv.namedWindow(title_erosion_window)
    cv.createTrackbar(title_trackbar_element_shape, title_erosion_window, 0, max_elem, erosion)
    cv.createTrackbar(title_trackbar_kernel_size, title_erosion_window, 0, max_kernel_size, erosion)
    cv.createTrackbar(title_trackbar_gaussian, title_erosion_window, 1, max_kernel_size, erosion)
...
def erosion(val):
    ...
    gaussian_size = cv.getTrackbarPos(title_trackbar_gaussian, title_erosion_window)
    ...
    erosion_dst = cv.GaussianBlur(erosion_dst, (gaussian_size,gaussian_size), 0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input', help='Path to input image.', default='OCR\pass1_1.jpg')
